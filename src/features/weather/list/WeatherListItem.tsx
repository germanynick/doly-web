...
export const WeatherListItem: React.FunctionComponent<IWeatherListItemProps> = (
  props
) => {
  const {
  ...
  const { loading, value } = useAsync(
    () => weatherService.weatherByLocation(woeid),
    [woeid]
  )

  const items =
    value?.consolidated_weather || times(6, () => ({} as IConsolidatedWeather))
          ...
          <Col key={index} span={4} className="weather-item">
            <Card loading={loading}>
              <Space direction="vertical">
                <Typography.Text>
                  {moment(item?.applicable_date, "YYYY-MM-DD").format("dddd")}
                </Typography.Text>
                <img
                  alt="state"
